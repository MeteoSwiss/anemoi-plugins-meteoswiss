...
def tests_summary (blacklist,weights,tests):
    ...
    return df
#------------------------------------------------
def write_summary(Myfile,df,para,dwh_para,weight):
    """
    Writes a summary into a .csv file based on certain conditions.

    Args:
    Myfile (str): The path of the file to be written.
    df (pandas.DataFrame): The DataFrame to be analyzed.
    para (str): A string parameter.
    dwh_para (str): Another string parameter.
    weight (int or float): The threshold weight for filtering values in the DataFrame.

    Returns:
    None

    Behavior:
    If the number of rows in the DataFrame (df) is greater than 0, the function writes a summary into a .csv file.
    The function logs the DataFrame and Myfile using the logging module.
    It iterates through the rows of the DataFrame and checks if any values in each row are greater than the specified weight.
    If any values are greater than the weight, the function appends specific information to the Myfile.
    If no values are greater than the weight in a row, the function logs 'no blacklisted values'.
    The function logs each step's information using the logging module.
    """
    if df.shape[0]>0:
        ...
        for l in range(0,df.shape[0]):
            ...
            for s in df.columns:
                if df[s][l]>weight:
                    ...
                    lista.append(s)
            if n>0:
                txt=df.index[l]+','+para+','+dwh_para+','+','.join(lista)+'\n'
                with open(Myfile, "a") as myfile:
                    myfile.write(txt)
            else:
                txt='no blacklisted values'
            LOG.info(txt)

#---------------------------------------------
def parse_jason_dir(path,para,tests,weights):
    """
    Parse the JSON files in the specified directory 'path', extracting relevant information and constructing two DataFrames, 'df_titan' and 'df_dwh'.
    ...
    Returns:
    tuple: A tuple containing two pandas DataFrames, 'df_titan' and 'df_dwh', which represent the parsed data from the JSON files.

    The function first parses 'titan.json' files in the specified directory and constructs a DataFrame 'df_titan' based on the extracted information. 
    It then processes 'dwh_flag.json' files, constructing a DataFrame 'df_dwh' similarly. 
    The function returns 'df_titan' and 'df_dwh' as the final results.

    Note: 'tests_summary' is assumed to be a function that is defined elsewhere in the code.

    Example:
    >>> path="/scratch/mbuzzi/inca/QC/tests_real/"
    >>> df_titan, df_dwh = parse_jason_dir(path)
    """
    LOG.info('Parsing '+str(path))
    black_stations=[]
    black_times=[]
    titan_jason_files=[]
    #loop over all jason files
    lst=sorted(os.listdir(path))
    for x in lst:
        #titan lib produced jason
        if x.endswith(para+"_titan.json"):
            titan_jason_files.append(x)
            LOG.info(x)
            f = open(str(path)+'/'+x)
            blacklist=json.load(f)
            #get weights and tests
            tests_tmp=[]
            weights_tmp=[]
            for te in blacklist['tests']:
                index=tests.index(te) if te in tests else -1
                if not index==-1:
                    w=weights[index]
                    tt=tests[index]
                    weights_tmp.append(w)
                    tt=tests_tmp.append(tt)
            df=tests_summary(blacklist,weights_tmp,tests_tmp)
            if df.shape[0]>0:
                cols=list(df.columns)
                for i in range(0,len(cols)):
                    times=list(df.index)
                    for k in range(0,len(times)):
                        if df.loc[:,cols[i]].iloc[k] > c.titan_ntests_threshold[para]['threshold_summary']:
                            black_times.append(times[k])
                            black_stations.append(cols[i]) 
    times=np.unique(black_times)
    stations=np.unique(black_stations)
    qc=np.zeros((len(times),len(stations)))
    ns=0
    for sta in stations:
        k=0
        index_sta=np.array([i for i,val in enumerate(black_stations) if val==sta])
        times_tmp=[black_times[i] for i in index_sta]
        for time in times:
            if times_tmp.count(time)>0:
                qc[k,ns]=1.0
            k=k+1
        ns=ns+1
    df_titan=pd.DataFrame(data=qc,index=times,columns=stations)
 
    cols=[]
    times=[]
    black_stations=[]
    black_times=[]
    #loop over all jason files DWH
    lst=sorted(os.listdir(path))
    for x in lst:
        if x.endswith(para+"_dwh_flag.json"):
            LOG.info(x)
            #dwh flag produced jason
            k = open(str(path)+'/'+x)
            blacklist=json.load(k)
            df=tests_summary(blacklist,[1],['DWH_flag'])
            if df.shape[0]>0:
                cols=list(df.columns)
                for i in range(0,len(cols)):
                    times=list(df.index)
                    for k in range(0,len(times)):
                        if df.loc[:,cols[i]].iloc[k]>0:
                            black_times.append(times[k])
                            black_stations.append(cols[i])            
    times=np.unique(black_times)
    stations=np.unique(black_stations)
    ...
    for sta in stations:
        ...
        for time in times:
            ...
            k=k+1
        ns=ns+1
    df_dwh=pd.DataFrame(data=qc,index=times,columns=stations)
    return df_titan, df_dwh,titan_jason_files
#--------------------------------------------------------------
